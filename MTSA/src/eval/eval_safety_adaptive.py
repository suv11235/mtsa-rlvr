...
from src.judge.strongreject_judge import StrongReject_Judge
...
def run_adaptive_eval(model, tokenizer, attacker_model, attacker_tokenizer, judge, dataset, num_samples=10, max_turns=3):
    ...
    for i in range(min(num_samples, len(dataset))):
        goal = dataset[i]['goal']
        print(f"\n" + "="*80, flush=True)
        print(f"--- [ADAPTIVE ATTACK {i+1}/{num_samples}] ---", flush=True)
        ...
        for turn in range(1, max_turns + 1):
            ...
            print(f"  ATTACK: {attack_question[:200]}...", flush=True)
            
            # 2. Get Response from Victim (model under test)
            # Try catch for generation errors
            try:
                victim_history_step = list(victim_history)
                victim_history_step.append({"role": "user", "content": attack_question})
                input_ids = tokenizer.apply_chat_template(victim_history_step, return_tensors="pt", add_generation_prompt=True).to(model.device)
        ...
        results.append({
            ...
            "turn_reached": turn if is_successful else max_turns,
            "final_response": final_response
        })

        # Progress reporting
        if (i + 1) % 10 == 0:
            current_asr = compliance_count / (i + 1)
            print(f"\n>>> [PROGRESS] Samples: {i+1}, Current ASR: {current_asr:.2%}")
...
def main():
    ...
    parser.add_argument("--judge_type", type=str, default="strongreject", choices=["llamaguard", "strongreject"])
    parser.add_argument("--dataset_path", type=str, default="datasets/attack_target/train_attack_target_labels.json")
    parser.add_argument("--num_samples", type=int, default=50)
    parser.add_argument("--max_turns", type=int, default=3)
    parser.add_argument("--skip_baseline", action="store_true")
    parser.add_argument("--unique_goals", action="store_true", default=True)
    ...
    if args.judge_type == "strongreject":
        judge = StrongReject_Judge(args.judge_model, gpu=victim_device, token=token)
    else:
    ...
    if args.unique_goals:
        print(">>> Filtering for UNIQUE goals...")
        import random
        random.seed(args.seed)
        unique_goals = list(set([item['goal'] for item in full_dataset]))
        random.shuffle(unique_goals)
        dataset = [{"goal": goal} for goal in unique_goals]
        print(f">>> Found {len(dataset)} unique goals.")
    else:
        dataset = full_dataset

    baseline_asr = 0.74 # Default reference
    if not args.skip_baseline:
        # 4. Eval Baseline
        print(f"\n>>> Evaluating BASELINE: {args.baseline_model}")
        tok_path = args.tokenizer_name_or_path if args.tokenizer_name_or_path else args.baseline_model
        ...
        model = model.to(victim_device).eval()
